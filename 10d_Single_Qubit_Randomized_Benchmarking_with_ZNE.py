...
class Parameters(NodeParameters):
    ...
    # Which primitive gates should get ZNE folding?
    # "x"   -> only x180/x90/-x90
    # "y"   -> only y180/y90/-y90
    # "none"-> disable folding entirely
    zne_target: Literal["x", "y", "none"] = "x"
...
node = QualibrationNode(
    name="10d_Single_Qubit_Randomized_Benchmarking_with_ZNE",
...
def _xy_dagger(label: str) -> str:
    """
    Return the adjoint pulse label for each X/Y primitive.

    Assumptions:
        - 'x180' and 'y180' are self-inverse.
        - 'x90' <-> '-x90'
        - 'y90' <-> '-y90'
    """
    ...
    if label == "-y90":
        return "y90"
    # Fallback: unknown labels are returned unchanged
    return label


def _is_x_gate(label: str) -> bool:
    return label in ("x180", "x90", "-x90")


def _is_y_gate(label: str) -> bool:
    return label in ("y180", "y90", "-y90")


def xy_pulse(qubit: Transmon, label: str, scale: int, zne_target: str) -> None:
    """
    Apply an XY pulse 'label' to 'qubit' with ZNE noise scale 'scale',
    respecting the zne_target selector:

        zne_target = "x"    -> fold only X gates
        zne_target = "y"    -> fold only Y gates
        zne_target = "none" -> no folding at all

    For odd scale λ = 2k + 1, implement:
        G -> G (G† G)^k
    where G is the pulse 'label' and G† is its adjoint. For λ = 1, just play G.
    """

    # Decide if this gate should be folded at all
    if zne_target == "none":
        qubit.xy.play(label)
        return
    if zne_target == "x" and not _is_x_gate(label):
        qubit.xy.play(label)
        return
    if zne_target == "y" and not _is_y_gate(label):
        qubit.xy.play(label)
        return

    # No folding if scale <= 1
    if scale <= 1:
        qubit.xy.play(label)
        return

    # Enforce odd λ for folding
    if scale % 2 != 1:
        raise ValueError(f"ZNE scale must be odd, got {scale}")

    dagger = _xy_dagger(label)
    k = (scale - 1) // 2

    # First logical gate
    qubit.xy.play(label)
    # Then k repetitions of G† G
    for _ in range(k):
        qubit.xy.play(dagger)
        qubit.xy.play(label)


def play_sequence(sequence_list, depth, qubit: Transmon, zne_scale: int, zne_target: str):
    i = declare(int)
    with for_(i, 0, i <= depth, i + 1):
        with switch_(sequence_list[i], unsafe=True):
            with case_(0):
                # Identity / wait – do NOT fold
                qubit.xy.wait(qubit.xy.operations["x180"].length // 4)
            with case_(1):  # x180
                xy_pulse(qubit, "x180", zne_scale, zne_target)
            with case_(2):  # y180
                xy_pulse(qubit, "y180", zne_scale, zne_target)
            with case_(3):  # Z180
                xy_pulse(qubit, "y180", zne_scale, zne_target)
                xy_pulse(qubit, "x180", zne_scale, zne_target)
            with case_(4):  # Z90 X180 Z-180
                xy_pulse(qubit, "x90", zne_scale, zne_target)
                xy_pulse(qubit, "y90", zne_scale, zne_target)
...
# %% {QUA_program builders}
def build_randomized_benchmarking_individual(zne_scale: int):
    zne_target = node.parameters.zne_target
                        ...
                        with for_(n, 0, n < n_avg, n + 1):
                            ...
                            if strict_timing:
                                with strict_timing_():
                                    ...
                                    play_sequence(sequence_list, depth, qubit, zne_scale, zne_target)
                            else:
                                play_sequence(sequence_list, depth, qubit, zne_scale, zne_target)
...
def build_randomized_benchmarking_multiplexed(zne_scale: int):
    zne_target = node.parameters.zne_target
    ...
    return randomized_benchmarking_multiplexed


# %% {Simulate_or_execute}
zne_results = None  # will be populated in the execution branch for fresh runs

if node.parameters.simulate:
    randomized_benchmarking = build_randomized_benchmarking_individual(zne_scale=1)
    ...
    # Fresh hardware run with ZNE sweep over λ
    node.results = {}
    zne_results = {}
    zne_summary = {}

    for zne_scale in ZNE_SCALES:
        print(f"Running X/Y RB for λ = {zne_scale}")
        if not node.parameters.multiplexed:
            randomized_benchmarking = build_randomized_benchmarking_individual(zne_scale=zne_scale)
        else:
            randomized_benchmarking = build_randomized_benchmarking_multiplexed(zne_scale=zne_scale)
        ...
        # %% {Data_fetching_and_dataset_creation for this λ}
        ...
        # JSON-safe per-λ summary
        lambda_key = f"lambda_{zne_scale}"
        zne_summary[lambda_key] = {}
        for q in qubits:
            qname = q.name
            zne_summary[lambda_key][qname] = {
        ...
        if zne_scale == 1:
            ...
            for q in qubits:
                qname = q.name
                node.results["fit_results"][qname] = {}
                node.results["fit_results"][qname]["EPC"] = EPC.sel(qubit=qname).values
                node.results["fit_results"][qname]["EPG"] = EPG.sel(qubit=qname).values
                print(f"λ={zne_scale}, {qname}: EPC={EPC.sel(qubit=qname).values}")
                print(f"λ={zne_scale}, {qname}: EPG={EPG.sel(qubit=qname).values}")
        else:
            for q in qubits:
                qname = q.name
                print(f"λ={zne_scale}, {qname}: EPC={EPC.sel(qubit=qname).values}")
    ...
    # After sweeping all λ, store the JSON-safe ZNE summary
    ...
    # Recompute analysis to populate local variables for plotting
    da_state = 1 - ds["state"].mean(dim="sequence")
    da_state: xr.DataArray
    da_state.attrs = {"long_name": "p(0)"}
    da_state = da_state.assign_coords(depths=da_state.depths - 1)
    ...
    if node.parameters.load_data_id is None:
        ...
        for ax, qubit in grid_iter(grid):
            ...
            for idx, zne_scale in enumerate(ZNE_SCALES):
                ...
                da_state_std = ds["state"].std(dim="sequence").sel(qubit=qname) / np.sqrt(ds.sequence.size)
                ...
                fit_dict = {k: da_fit.sel(qubit=qname).sel(fit_vals=k).values for k in da_fit.fit_vals.values}
                ax.plot(
                    m_vals,
        ...
        plt.tight_layout()
        plt.suptitle("X/Y RB with ZNE", y=1.02)

    else:
        # Single-λ plotting (load_data_id path)
        grid_names = [q.grid_location for q in qubits]
        ...
        plt.tight_layout()
        plt.suptitle("X/Y RB (λ=1)", y=1.02)
